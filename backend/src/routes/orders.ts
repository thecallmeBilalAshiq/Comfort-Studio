...
const getOrderItems = (orderId: number) => {
  return db.prepare(`
    SELECT oi.id, oi.orderId, oi.productId, oi.quantity, oi.price, 
           oi.selectedSize, oi.selectedColor, oi.selectedStorage, oi.selectedMattress,
...
router.post('/', optionalAuthMiddleware, (req: AuthRequest, res) => {
  try {
    ...
    for (const item of items) {
      ...
      const itemPrice = item.price ? Number(item.price) : product.price;
    ...
    res.json({ ...order, items: getOrderItems(orderId as number) });
  } catch (e: any) {
